...
namespace XIVSlothCombo.Combos.PvE
{
    internal class RPR
    {
        ...
        internal class RPR_HoldBurstCombo : CustomCombo
        {
            protected internal override CustomComboPreset Preset { get; } = CustomComboPreset.RPR_HoldBurstCombo;

            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                RPRGauge? gauge = GetJobGauge<RPRGauge>();

                if (actionID is ShadowOfDeath && TargetHasEffect(Debuffs.DeathsDesign))
                {
                    ...
                    if (LevelChecked(BloodStalk) && !HasEffect(Buffs.Enshrouded) && gauge.Soul >= 50 &&
                        (!LevelChecked(Gluttony) ||
                        (LevelChecked(Gluttony) && (gauge.Soul is 100 || GetCooldownRemainingTime(Gluttony) >= 10))))
                        return OriginalHook(BloodStalk);

                    if (LevelChecked(ShadowOfDeath) && !HasEffect(Buffs.SoulReaver) && GetDebuffRemainingTime(Debuffs.DeathsDesign) <= 6)
                        return ShadowOfDeath;

                    if (!HasEffect(Buffs.ImmortalSacrifice) && !HasEffect(Buffs.Enshrouded) && !HasEffect(Buffs.SoulReaver) &&
                       ActionReady(SoulSlice) && gauge.Soul <= 50)
                        return SoulSlice;

                    if (HasEffect(Buffs.SoulReaver) && LevelChecked(Gibbet))
                    {
